...
parser$add_argument("--start_pi1", default=5e-4, type="double", help="Start value for Mixing proportion for Y1-only SNPs")
parser$add_argument("--start_pi2", default=5e-4, type="double", help="Start value for Mixing proportion for Y2-only SNPs")
parser$add_argument("--start_piC", default=5e-4, type="double", help="Start value for Mixing proportion for pleiotropic SNPs")
parser$add_argument("--start_hsq_gamma1", default=-100, type="double", help="Start value for Variance explained by Y1-only SNPs on Y1")
parser$add_argument("--start_hsq_gamma2", default=-100, type="double", help="Start value for Variance explained by Y2-only SNPs on Y2")
parser$add_argument("--start_hsq_gammaC1", default=-100, type="double", help="Start value for Variance explained by pleiotropic SNPs on Y1")
parser$add_argument("--start_hsq_gammaC2", default=-100, type="double", help="Start value for Variance explained by pleiotropic SNPs on Y2")
parser$add_argument("--start_covGammaC", default=-100, type="double", help="Start value for Coefficients for cov.gammaC")
...
parser$add_argument("--sefun_opt", default=0, type="integer", help="run SEfun in step1-gen: 0 (false,default) / non-zeros (true)")
...
parser$add_argument("--tol_pi_thresh", default=0.05, type="double", help="threshold for converge")
parser$add_argument("--tol_sigma_thresh", default=0.05, type="double", help="threshold for converge")
parser$add_argument("--tol_cov_thresh", default=0.05, type="double", help="threshold for converge")
parser$add_argument("--tol_delta_thresh", default=0.05, type="double", help="threshold for converge")
parser$add_argument("--tol_stratification_thresh", default=0.05, type="double", help="threshold for converge")
parser$add_argument("--tol_stratifiCovariance_thresh", default=0.05, type="double", help="threshold for converge")
...
model <- args$model
sefun_opt <- args$sefun_opt
randinit_opt <- 1 # always set as 1 in step1
randinit_nbo <- 1 # always set as 1 in step1
...
rg_gen_est <- 0.0; # no use @Step1


#------------------------------------------------------------------
### Starting values
#------------------------------------------------------------------
inichk <- checkiniparam(args$start_pi1, args$start_pi2, args$start_piC,
						args$start_delta12, args$start_delta21, 
						args$start_hsq_gamma1, args$start_hsq_gamma2, 
						args$start_hsq_gammaC1, args$start_hsq_gammaC2, args$start_covGammaC,
						args$start_stratification1, args$start_stratification2, args$start_stratifiCovariance,
						model);

start_pi1 <- inichk[1]; start_pi2 <- inichk[2]; start_piC <- inichk[3];
start_delta12 <- inichk[4]; start_delta21 <- inichk[5];
start_hsq_gamma1 <- inichk[6]; start_hsq_gamma2 <- inichk[7]; 
start_hsq_gammaC1 <- inichk[8]; start_hsq_gammaC2 <- inichk[9]; start_covGammaC <- inichk[10];
start_var.gamma1 <- inichk[11]; start_var.gamma2 <- inichk[12];
start_var.gammaC1 <- inichk[13]; start_var.gammaC2 <- inichk[14]; start_cov.gammaC <- inichk[15];
start_stratification1 <- inichk[16]; start_stratification2 <- inichk[17]; start_stratifiCovariance <- inichk[18];
start_ldsc_hsq1 <- inichk[19]; start_ldsc_hsq2 <- inichk[20]; start_ldsc_cov <- inichk[21]; 


### Set the LDmatrix
Nkcomb <- nbomatfun(neibo_cau_num, model);
cat("  --> Compiling EMgen Rcpp: ", model, "\n")
sourceCpp(EM_Rcpp_file);


### Total hsq
start_total_hsq1 <- start_ldsc_hsq1; start_total_hsq2 <- start_ldsc_hsq2;
